...
import type { GenreMovieList200GenresItem, GenreTvList200GenresItem } from '@/api/types';
...
export const useGenresStore = defineStore('genres', () => {
  ...
  const fetchGenresMovies = async () => {
    try {
      ...
      console.log('Failed to fetch genres movies', err);
      throw err;
    }
  }
  ...
  const fetchGenresTv = async () => {
    try {
      ...
      console.log('Failed to fetch genres TV', err);
      throw err;
    }
  }
